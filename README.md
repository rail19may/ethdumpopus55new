...
6. **Детект.** Для каждого пула в памяти хранятся цены за последние `window_blocks` блоков. Сигнал
   ...
   Сравниваются только состояния пулов **на конец блока**. Промежуточные состояния внутри блока
   (флеш-заёмы, сэндвичи, «донат + swap» в одной транзакции) существуют мгновение и давали бы ложные
   «дампы» с миллионами долларов ликвидности, поэтому детектор их не видит.

   Цена, действовавшая на начало окна, тоже учитывается. Поэтому дамп в редко торгуемом пуле
   (прошлая сделка 100 блоков назад) не теряется. Для пула, по которому у бота ещё нет данных,
   состояние на конец предыдущего блока читается напрямую: V2 — `getReserves()`, V3 — `slot0()` и
   `balanceOf`.
7. **Пометки в алерте:**
...
| `rpc.poll_quiet_after_block_sec` | `10` | после нового блока столько секунд не опрашивать (блоки — раз в 12 с; экономит лимит RPC) |
